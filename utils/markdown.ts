...
import { VolumeFrontmatter, type VolumeFrontmatterType, type VolumeMetaSchemaType, type VolumeToCType } from './schema';
...
export function fromMarkdownToMdast(
    fileContent: string
): MdastRoot {
...
export function hastToHtml(
    ...
    switch (template) {
        case 'chapter': {
            return chapterTemplate(current, meta, html, filename);
        }
        case 'insert': {
            return insertTemplate(current, meta, html, filename);
        }
        default: {
...
export function splitContentAtImage(
    mdastRoot: MdastRoot,
): (MdastRoot & { fullImage?: boolean })[] {
    ...
    for (let i = 0; i < mdastRoot.children.length; i++) {
        ...
        if (child.type === 'paragraph' && child.children.length === 1 && child.children[0]!.type === 'image') {
            ...
            if (!isNoSplit) { // If no split, do nothing
